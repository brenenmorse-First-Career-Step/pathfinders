...
export async function GET(request: NextRequest) {
    ...
    let q = supabase
        .from('resumes')
        .select('id, user_id, title, status, pdf_url, shareable_link, created_at, updated_at', { count: 'exact' })
    ...
    const resumesList = rows ?? [];
    const userIds = [...new Set(resumesList.map((r: { user_id: string }) => r.user_id))];
    ...
    const list = resumesList.map((r: { user_id: string }) => ({
        ...r,
